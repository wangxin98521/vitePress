...
|()|二选一, 分组,可以被提取出来|
|(?:exp)|非捕获分组,不会被提取出来|
|\n|回溯分组,n为第几个分组里的内容[匹配第n个分组的内容,以1开始]|
...
~~~javascript
let str = '01020342321010233';
str.replace(/2(?=3)/g, 6); // => '01020346321010633' [后面是3的2 换成 6]
str.replace(/2(?!3)/g, 6); // => '01060342361010233' [后面不是3的2 换成 6]
str.replace(/(?<=3)2/g, 6); // => '01020342361010233' [前面是3的2 换成 6]
str.replace(/(?<!3)2/g, 6); // => '01060346321010633' [前面不是3的2 换成 6]
~~~
...
/(?:https|http):\/\/?([\w.]+)+([\/[\w]+]*)\??(\S*)/g.exec(value); // url地址匹配
~~~
